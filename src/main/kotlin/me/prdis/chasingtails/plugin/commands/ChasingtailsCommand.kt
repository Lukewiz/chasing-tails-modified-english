    ...
    fun registerCommand() {
        plugin.lifecycleManager.registerEventHandler(LifecycleEvents.COMMANDS) { event ->
            ...
            ct.then(Commands.literal("start").executes {
                ...
                if (server.onlinePlayers.filter { player -> player.gameMode != GameMode.SPECTATOR }.size !in 3..8) {
                    ctx.sendMessage(text("플레이어의 수가 3~8명 이어야합니다.", NamedTextColor.RED))
                } else {
                    if (!isRunning) {
                        plugin.launch {
                            server.onlinePlayers.forEach { player ->
                                ...
                                delay(1000)
                                player.sendMessage(text("2초 후 게임이 시작됩니다."))
                                delay(1000)
                                player.sendMessage(text("1초 후 게임이 시작됩니다."))
                                delay(1000)
                                player.sendMessage(text(""))
                                player.sendMessage(text("게임을 시작중입니다."))
                                player.sendMessage(text("1~2분 정도 걸릴 수 있습니다."))
                            }
                            server.onlinePlayers.forEach { player ->
                                repeat(100) {
                                    player.sendMessage(text("\n"))
            ...
            registrar.register(ct.build(), "A chasing tails command. /ct", listOf("ct"))
        }
    }
}